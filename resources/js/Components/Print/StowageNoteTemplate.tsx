...
export default function StowageNoteTemplate({ order }: Props) {
    ...
    return (
        ...
        <div className="w-full h-full bg-white font-sans text-[10px] text-black leading-tight p-2 box-border">
            ...
            {/* --- HEADER --- */}
            <div className={`flex w-full ${borderClass} border-2 mb-1`}>
                {/* Logo Section */}
                <div className={`w-[15%] ${borderClass} border-r-2 p-1 flex flex-col items-center justify-center`}>
                    <img src="/images/logo_proagro.png" alt="ProAgro" className="h-8 object-contain mb-1" />
                    <div className="text-[7px] font-bold text-green-800 leading-none text-center">PRO <br /> AGROINDUSTRIA</div>
                </div>

                {/* Title Section */}
                <div className={`w-[75%] ${borderClass} border-r-2 flex flex-col items-center justify-center py-1`}>
                    <h1 className="text-lg font-bold uppercase">PRO-AGROINDUSTRIA, S.A. DE C.V.</h1>
                    <h2 className="text-base font-bold uppercase">GLS-AP-FO-002</h2>
                    <h2 className="text-lg font-bold uppercase">NOTA DE ESTIBA A CAMIÓN</h2>
                </div>

                {/* Meta Data Section */}
                <div className="w-[10%] flex flex-col text-[7px]">
                    <div className={`flex-1 flex ${borderClass} border-b`}>
                        <div className={`w-[40%] bg-green-100 font-bold flex items-center justify-center ${borderClass} border-r`}>FECHA</div>
                        <div className="w-[60%] flex items-center justify-center font-bold px-1">{order.date || order.created_at?.split('T')[0]}</div>
                    </div>
                    <div className={`flex-1 flex ${borderClass} border-b`}>
                        <div className={`w-[40%] bg-green-100 font-bold flex items-center justify-center ${borderClass} border-r`}>TURNO</div>
                        <div className="w-[60%] flex items-center justify-center px-1"></div>
                    </div>
                    <div className={`flex-1 flex ${borderClass}`}>
                        <div className={`w-[40%] bg-green-100 font-bold flex items-center justify-center ${borderClass} border-r`}>HORA</div>
                        <div className="w-[60%] flex items-center justify-center px-1"></div>
                    </div>
                </div>
            </div>
            ...
            <div className={`flex w-full ${borderClass} border-2 mb-1`}>
                <div className={`w-[30%] ${borderClass} border-r-2 flex flex-col`}>
                    <div className={headerClass}>NOMBRE Y FIRMA DEL CHOFER</div>
                    <div className="flex-1 flex items-center justify-center font-bold uppercase text-[10px] p-1">
                ...
                <div className="w-[70%] flex flex-col">
                    ...
                    <div className={`flex ${borderClass} border-b`}>
                        <div className={`w-[20%] text-[7px] bg-green-50 pl-1 ${borderClass} border-r flex items-center`}>NOMBRE DE LA FLETERA</div>
                        <div className={`w-[50%] font-bold uppercase px-1 flex items-center ${borderClass} border-r text-[9px]`}>
                            {order.transport_company || order.carrier?.name || "N/A"}
                        </div>
                        <div className={`w-[15%] text-[7px] bg-green-50 pl-1 ${borderClass} border-r flex items-center`}>TIPO DE UNIDAD</div>
                        <div className="w-[15%] font-bold text-center flex items-center justify-center uppercase text-[9px]">
                            {order.is_full ? 'FULL' : (order.unit_type || 'SENCILLO')}
                        </div>
                    </div>
                    <div className="flex h-5">
                        <div className={`w-[70%] font-bold uppercase px-1 flex items-center ${borderClass} border-r text-[9px]`}>
                        </div>
                        <div className={`w-[10%] text-[7px] bg-green-50 pl-1 ${borderClass} border-r flex items-center`}>PLACAS</div>
                        <div className="w-[20%] font-bold text-[7px] flex flex-col justify-center px-1 leading-none uppercase">
            ...
            {/* --- DATOS GENERALES --- */}
            <div className={`w-full ${borderClass} border-2 mb-1`}>
                <div className={headerClass}>DATOS GENERALES</div>
                <div className="flex h-8">
                    <div className={`w-[15%] ${borderClass} border-r`}>
                        <div className={`bg-green-50 text-[6px] pl-1 ${borderClass} border-b`}>ORDEN DE VENTA</div>
                        <div className="h-5 flex items-center justify-center font-bold">{order.sales_order?.sale_order || order.sales_order?.folio || "N/A"}</div>
                    </div>
                    <div className={`w-[15%] ${borderClass} border-r`}>
                        <div className={`bg-green-50 text-[6px] pl-1 ${borderClass} border-b`}>ORDEN DE EMBARQUE</div>
                        <div className="h-5 flex items-center justify-center font-bold text-xs">{order.folio}</div>
                    </div>
                    <div className={`w-[40%] ${borderClass} border-r`}>
                        <div className={`bg-green-100 text-center text-[6px] ${borderClass} border-b`}>DESTINATARIO (CLIENTE)</div>
                        <div className="flex h-5">
                            <div className={`w-[60%] ${borderClass} border-r flex flex-col`}>
                                <div className={`text-[6px] pl-1 bg-green-50 ${borderClass} border-b`}>NOMBRE</div>
                            ...
                            <div className="w-[40%] flex flex-col">
                                <div className={`text-[6px] pl-1 bg-green-50 ${borderClass} border-b`}>DESTINO</div>
                                <div className="flex-1 flex items-center justify-center font-bold uppercase text-[7px] leading-none px-1 text-center">
                    ...
                    <div className="w-[30%] flex flex-col">
                        <div className={`text-[6px] bg-green-100 text-center ${borderClass} border-b`}>SUPERVISOR</div>
                        <div className={`flex-1 pt-3 ${borderClass} border-b mx-2`}></div>
                    </div>
                </div>
            </div>
