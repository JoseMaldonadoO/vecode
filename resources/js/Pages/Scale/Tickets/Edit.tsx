...
export default function Edit({ auth, ticket, order }: { auth: any, ticket: any, order: any }) {
    ...
    useEffect(() => {
        const net = Math.abs((parseFloat(data.gross_weight) || 0) - (parseFloat(data.tare_weight) || 0));
    ...
    return (
        <DashboardLayout user={auth.user} header={`Editar Ticket #${ticket.ticket_number}`}>
            ...
            <div className="max-w-4xl mx-auto py-8 px-4">
                ...
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                    ...
                    <div className="bg-gray-50 p-6 border-b border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <span className="text-xs font-bold text-indigo-500 uppercase tracking-wider">Folio Orden</span>
                            <div className="text-2xl font-black text-gray-900">{order.folio}</div>
                            <div className="mt-2 text-sm text-gray-600">
                                <span className="font-bold">Cliente:</span> {order.client_name || order.client?.name || order.vessel?.client?.name || 'N/A'}
                            </div>
                        </div>
                        <div className="text-right">
                            <span className="text-xs font-bold text-green-600 uppercase tracking-wider">Estatus</span>
                            <div className="text-lg font-bold text-gray-800 uppercase">{ticket.weighing_status === 'completed' ? 'Completado' : 'En Progreso'}</div>
                            <div className="mt-2 text-sm text-gray-600">
                                <span className="font-bold">Chofer:</span> {order.operator_name || 'N/A'}
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={submit} className="p-8 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-indigo-50 rounded-xl border border-indigo-100">
                                ...
                                {/* Tare */}
                                <div>
                                    <InputLabel htmlFor="tare_weight" value="Peso Tara (Entrada)" className="text-indigo-900" />
                                    <TextInput
                                        id="tare_weight"
                                        type="number"
                                        ...
                                        required
                                    />
                                    <InputError message={errors.tare_weight} className="mt-2" />
                                </div>

                                {/* Gross */}
                                <div>
                                    <InputLabel htmlFor="gross_weight" value="Peso Bruto (Salida)" className="text-indigo-900" />
                                    <TextInput
                                        id="gross_weight"
                                        type="number"
                                        className="mt-1 block w-full font-mono text-lg font-bold text-center border-indigo-200 focus:border-indigo-500 focus:ring-indigo-500"
                                        ...
                                        required
                                    />
                                    <InputError message={errors.gross_weight} className="mt-2" />
                                </div>

                                {/* Net (Read Only usually, but editable if needed? logic says auto-calc) */}
                                <div>
                                    <InputLabel htmlFor="net_weight" value="Peso Neto (Calculado)" className="text-green-700" />
                                    <TextInput
                                        id="net_weight"
                                        type="number"
                            ...
                            <div className="mt-2 flex items-start gap-2 text-xs text-amber-600 bg-amber-50 p-2 rounded border border-amber-100">
                                ...
                                <p>Advertencia: Modificar los pesos manualmente afectará los reportes y el inventario. Asegúrese de tener autorización.</p>
                            </div>
                        </div>

                        {/* Observations */}
                        <div>
                            <InputLabel htmlFor="observations" value="Observaciones / Motivo de Edición" />
                            <textarea
                                id="observations"
                        ...
                        <div className="flex items-center justify-end pt-6 border-t border-gray-100">
                            ...
                            <PrimaryButton disabled={processing} className="px-6 py-3 text-base">
