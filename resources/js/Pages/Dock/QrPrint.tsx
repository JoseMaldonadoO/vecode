...
export default function QrPrint({ auth }: { auth: any }) {
    ...
    useEffect(() => {
        const search = async () => {
            if (query.length > 2) {
                try {
                    const response = await axios.get(`/dock/operators/search?q=${query}`);
            ...
            <div className="max-w-4xl mx-auto print:hidden">
                ...
                {!selectedOperator ? (
                    <div className="bg-white rounded-xl shadow-lg p-8">
                        <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Buscar Operador</h2>
                        ...
                        <div className="mt-6 space-y-2 max-w-lg mx-auto">
                            ...
                            {query.length > 2 && results.length === 0 && (
                                <p className="text-center text-gray-500 py-4">No se encontraron operadores.</p>
                            )}
                        </div>
                    </div>
                ) : (
                    <div className="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <div className="items-center justify-center flex flex-col bg-gray-100 p-8 rounded-xl border border-dashed border-gray-300">
                            <p className="text-gray-500 mb-4 text-sm font-medium uppercase tracking-wide">Vista Previa</p>
                            {/* Card Container for Preview */}
                            <div className="bg-white w-[8.5in] min-h-[500px] shadow-2xl p-16 relative"> {/* Approx letter width scale */}
                                {/* Header Logo */}
                                <div className="mb-8">
                                    <img src="/logo_pro_agro.png" alt="Pro Agroindustria" className="h-20 object-contain" />
                                </div>

                                <h1 className="text-3xl font-bold text-gray-900 mb-8 font-sans">Datos de Unidad</h1>

                                <div className="space-y-6 text-lg font-serif">
                                    <p className="tracking-wide"><span className="font-bold">ECONOMICO:</span> {selectedOperator.economic_number}</p>
                                    <p className="tracking-wide"><span className="font-bold">TIPO DE UNIDAD:</span> {selectedOperator.unit_type.toUpperCase()}</p>
                                    {/* Modelo mocked as not in DB yet */}
                                    {/* <p className="tracking-wide"><span className="font-bold">MODELO:</span> 2015</p> */}
                                    <p className="tracking-wide"><span className="font-bold">PLACA:</span> {selectedOperator.tractor_plate}</p>
                                    <p className="tracking-wide"><span className="font-bold">LINEA TRANSPORTISTA:</span> {selectedOperator.transporter_line}</p>
                                    <p className="tracking-wide"><span className="font-bold">NOMBRE DEL OPERADOR:</span> {selectedOperator.operator_name}</p>
                                </div>
            ...
            <div className="hidden print:block fixed inset-0 bg-white z-50 p-12">
                {selectedOperator && (
                    <div className="max-w-[21cm] mx-auto">
                        <div className="mb-8">
                            <img src="/logo_pro_agro.png" alt="Pro Agroindustria" className="h-24 object-contain" />
                        </div>

                        <h1 className="text-4xl font-bold text-black mb-10 font-sans">Datos de Unidad</h1>

                        <div className="space-y-6 text-xl font-serif text-black">
                            <p><span className="font-bold">ECONOMICO:</span> {selectedOperator.economic_number}</p>
                            <p><span className="font-bold">TIPO DE UNIDAD:</span> {selectedOperator.unit_type.toUpperCase()}</p>
                            <p><span className="font-bold">PLACA:</span> {selectedOperator.tractor_plate}</p>
                            <p><span className="font-bold">LINEA TRANSPORTISTA:</span> {selectedOperator.transporter_line}</p>
                            <p><span className="font-bold">NOMBRE DEL OPERADOR:</span> {selectedOperator.operator_name}</p>
                        </div>

                        <div className="mt-20 flex justify-center">
                            {qrDataUrl && <img src={qrDataUrl} alt="QR Code" className="w-80 h-80" />}
                        </div>
                    </div>
                )}
            </div>
