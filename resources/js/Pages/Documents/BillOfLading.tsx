        ...
        <div className="bg-white min-h-screen p-8 text-gray-900 font-sans text-sm">
            ...
            <div className="max-w-[216mm] mx-auto border border-gray-300 p-8 print:border-none print:p-0">
                ...
                <div className="flex justify-between items-start border-b-2 border-gray-800 pb-4 mb-6">
                    <div>
                        <h1 className="text-2xl font-bold uppercase tracking-wider">Carta Porte</h1>
                        <p className="text-gray-500 text-xs mt-1">Autotransporte Federal de Carga</p>
                    </div>
                    <div className="text-right">
                        <div className="bg-gray-100 p-2 rounded border border-gray-200">
                            <p className="text-xs text-gray-500 font-bold uppercase">Folio</p>
                            <p className="text-xl font-mono font-bold text-red-600">{order.folio}</p>
                        </div>
                        <p className="text-xs mt-2">Fecha: {new Date().toLocaleDateString()}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 mb-8">
                    ...
                    <div className="border p-4 rounded-lg">
                        <h3 className="font-bold text-gray-700 border-b pb-2 mb-2 flex items-center uppercase text-xs tracking-wide">
                            <MapPin className="w-3 h-3 mr-1" /> Origen / Remitente
                        </h3>
                        <p className="font-bold text-lg">VECODE PLANT</p>
                        <p>Carretera Federal 45 Km 10</p>
                        <p>Celaya, Guanajuato, CP 38000</p>
                        <p className="text-xs text-gray-500 mt-2">RFC: VEC-900101-000</p>
                    </div>

                    {/* Destination / Consignee */}
                    <div className="border p-4 rounded-lg">
                        <h3 className="font-bold text-gray-700 border-b pb-2 mb-2 flex items-center uppercase text-xs tracking-wide">
                            <MapPin className="w-3 h-3 mr-1" /> Destino / Destinatario
                        </h3>
                        <p className="font-bold text-lg">{order.client?.business_name}</p>
                        <p>{order.client?.address || 'Domicilio Conocido'}</p>
                        <p className="text-xs text-gray-500 mt-2">RFC: {order.client?.rfc}</p>
                    </div>
                </div>
                ...
                <div className="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <p className="text-xs text-gray-500 uppercase">Transportista</p>
                            <p className="font-bold">{order.transporter?.name}</p>
                        </div>
                        <div>
                            <p className="text-xs text-gray-500 uppercase">Operador</p>
                            <p className="font-bold">{order.driver?.name}</p>
                            <p className="text-xs">{order.driver?.license_number}</p>
                        </div>
                        <div>
                            <p className="text-xs text-gray-500 uppercase">Vehículo</p>
                            <p className="font-bold">{order.vehicle?.plate_number}</p>
                        ...
                        <tbody>
                            {order.items?.map((item: any, idx: number) => (
                                <tr key={idx} className="border-b border-gray-100">
                                    <td className="p-2 font-mono text-xs">{item.product?.code}</td>
                                    <td className="p-2 font-bold">{item.product?.name}</td>
                                    <td className="p-2">{item.packaging_type}</td>
                ...
                <div className="mt-12 text-center text-[10px] text-gray-400">
                    <p>Este documento es una representación impresa de un CFDI de Traslado (Demo VECODE).</p>
                    <p>La reproducción no autorizada de este comprobante constituye un delito en los términos de las disposiciones fiscales.</p>
                </div>
            </div>
