...
try {
    ...
    // 3. Migrate Data
    $count = DB::table('shipment_orders')->count();
    $loaded = DB::table('loading_orders')->count();

    echo "Shipment Orders Found: $count<br>";
    echo "Loading Orders Found: $loaded<br>";
    ...
    if ($loaded < $count) {
        ...
        foreach ($orders as $order) {
            // Check if exists
            if (DB::table('loading_orders')->where('id', $order->id)->exists()) {
                continue;
            }

            // Fetch product from items manually
        ...
        echo "Data appears to be already migrated.<br>";
    }
